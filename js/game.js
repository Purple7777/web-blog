...
//hub lock
var hublock_arr = [{
    image: 'assets/hub_lock.png',
    power: 'assets/hub_lock_power.png',
    indicator: 'assets/hub_lock_indicator.png',
    indicatorPower: 'assets/hub_lock_indicator_power.png'
}, {
    image: 'assets/hub_lock_supply.png',
    power: 'assets/hub_lock_supply_power.png',
    indicator: 'assets/hub_lock_indicator_supply.png',
    indicatorPower: 'assets/hub_lock_indicator_power_supply.png'
}];

//hub type
var hub_arr = [{
        type: 'power',
        image: 'assets/hub_power.png',
        power: 'assets/hub_power.png',
        range: 70,
        lock: 0
    },
    {
        type: 'supply',
        image: 'assets/hub_supply.png',
        power: 'assets/hub_supply_power.png',
        range: 47,
        lock: 1
    },
    {
        type: 'linear',
        image: 'assets/hub_linear.png',
        power: 'assets/hub_linear_power.png',
        range: 30,
        lock: 0
    },
    {
        type: 'corner',
        image: 'assets/hub_corner.png',
        power: 'assets/hub_corner_power.png',
        range: 30,
        lock: 0
    },
    {
        type: 'cross',
        image: 'assets/hub_cross.png',
        power: 'assets/hub_cross_power.png',
        range: 30,
        lock: 0
    },
    {
        type: 't',
        image: 'assets/hub_t.png',
        power: 'assets/hub_t_power.png',
        range: 30,
        lock: 0
    }
];
...
function buildGameButton() {
    ...
    y8logo.cursor = "pointer";
    y8logo.addEventListener("mousedown", function(evt) {
        openY8Logo()
    });
    y8logoGameOver.cursor = "pointer";
    y8logoGameOver.addEventListener("mousedown", function(evt) {
        openY8Logo()
    });
    y8LogoSponsorCheck()

    LeaderboardBtn.cursor = "pointer";
    LeaderboardBtn.addEventListener("mousedown", function(evt) {
        playSound('soundClick', false);
        _showLeaderboard()
    });

    LeaderboardBtn1.cursor = "pointer";
    LeaderboardBtn1.addEventListener("mousedown", function(evt) {
        playSound('soundClick', false);
        _showLeaderboard()
    });
...
function y8LogoSponsorCheck() {
    if (s_sponsor === true && (curPage === "main" || curPage === "result")) {
        y8logo.cursor = "initial";
        y8logoGameOver.cursor = "initial";
    }
}

function openY8Logo() {
    console.log('openY8Logo')
    if (s_sponsor === false) {
        playSound('soundClick', false);
    }
    if (s_sponsor === true) {
...
function idCallback(response) {
    if (response) {
        if (response.status === 'ok') {
            LeaderboardBtn1.visible = false;
            LeaderboardBtn.visible = true;
            s_userName = response.authResponse.details.nickname;
...
function submitScore() {
    console.log('submitScore ' + totalScoreGame)
    ID.GameAPI.Leaderboards.save({
        'table': 'Leaderboard',
        'points': totalScoreGame
    });
}
...
function goPage(page) {
    ...
    switch (page) {
        ...
        case 'result':
            ...
            console.log("totalScoreGame " + totalScoreGame)
            y8LogoSponsorCheck()
            if (s_isLogin === false) {
                LeaderboardBtn.visible = false;
                LeaderboardBtn1.visible = true;
            } else {
                LeaderboardBtn.visible = true;
                LeaderboardBtn1.visible = false;
            }
            s_iScore = gameData.stageNum;
            submitScore()
            try {
                console.log('Showing Ads')
                nextAds()
...
function prepareStage() {
    totalScoreGame = scoreGame + totalScoreGame;
    scoreGame = 50;
...
function endGame() {
    stopTimerSound();
    gameData.pause = true;
    TweenMax.to(stageAnnounceContainer, 3, {
        overwrite: true,
        onComplete: function() {
            gameData.stageNum++;
    ...
    if (!gameData.pause) {
        ...
        for (var ch = 0; ch < stage_arr[gameData.stageNum].hub.length; ch++) {
            if (stage_arr[gameData.stageNum].hub[ch].trick == 2 && $hub[ch + '_timer_bar'].visible) {
                ...
                $hub[ch + '_timer_text'].text = millisecondsToTime($hub[ch + '_timer_text'].timerEnd - $hub[ch + '_timer_text'].timerDistance);

                if ($hub[ch + '_timer_text'].timerEnd - $hub[ch + '_timer_text'].timerDistance <= 0) {
                    $hub[ch + '_timer_text'].text = millisecondsToTime(0);
...
function displayMessage(con) {
    if (con == 'stage') {
        ...
        TweenMax.to(stageAnnounceContainer, .5, {
            alpha: 1,
            overwrite: true,
            onComplete: function() {
                TweenMax.to(stageAnnounceContainer, 2, {
                    alpha: 1,
                    overwrite: true,
                    onComplete: function() {
                        TweenMax.to(stageAnnounceContainer, .5, {
